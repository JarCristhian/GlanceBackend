...
export class WebsocketController implements OnGatewayConnection, OnGatewayDisconnect {
    ...
    sendMessageforAll(@MessageBody() data: any) {
        ...
        console.log('Received message for all:', data);
    }

    @SubscribeMessage('messageNotMe')
    sendMessageNotMe(@ConnectedSocket() client: Socket, @MessageBody() data: any) {
        client.broadcast.emit('messageServerNotMe', data);
        console.log('Received message of user:', data);
    }

    @SubscribeMessage('createLanguage')
    async createLanguage(@ConnectedSocket() client: Socket, @MessageBody() data: any) {
        console.log('Received message of user:', data);
        await this.websocketService.createLanguage(data, 1);
        client.broadcast.emit('updateLanguage', `Nuevo lenguage agregado!! ${data.name}`);
    }
}